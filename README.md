...
2. Create a package with your name and move `com.abas.Main.java` inside it:

```
src/com/<yourname>/com.abas.Main.java
```

For example, if your name is Franco: `src/com/franco/com.abas.Main.java`

The starter `com.abas.Main.java` contains TODOs to guide the first steps:
...
graph TB
    subgraph Presentation
        M[com.abas.Main.java<br>CLI Menu Loop]
    end
...
- Implement the CLI menu loop in `com.abas.Main.java`
...
**Wiring in com.abas.Main.java** — the student can now swap between array and file implementations for bookings:
...
File file = new File("src/com/amigoscode/User.csv");

// After - loads from classpath (src/main/resources/)
File file = new File(
    getClass().getClassLoader().getResource("User.csv").getPath()
);
```
...
These can be swapped in via dependency injection in `com.abas.Main.java`:
...
Place this bean in the `com.abas.Main` class (annotated with `@SpringBootApplication`). The data is inserted into H2 on every startup since the database is in-memory.
...
│   │   └── com/
│   │       └── <yourname>/
│   │           ├── com.abas.Main.java                       (@SpringBootApplication)
│   │           ├── car/
│   │           │   ├── Brand.java
│   │           │   ├── Car.java                    (@Entity)
